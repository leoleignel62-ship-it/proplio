...
export async function POST(request: Request) {
  try {
    ...
    const { data: bail, error: bailErr } = await supabase
      .from("baux")
      .select(
        "id, date_debut, irl_reference, loyer_initial, revision_loyer, loyer, date_derniere_revision, statut, proprietaire_id, logement_id, locataire_id",
      )
      .eq("id", bailId)
    ...
    const loyerAvant = Number((bail as { loyer?: unknown }).loyer ?? 0);
    const { nouveauLoyer } = calculerNouveauLoyer(loyerAvant, irlRef, irl.valeur);
    ...
    const { data: inserted, error: insErr } = await supabase
      ...
      .insert({
        ...
        loyer_avant: loyerAvant,
        loyer_apres: nouveauLoyer,
        irl_ancien: irlRef,
    ...
    const { error: upErr } = await supabase
      ...
      .update({
        loyer: nouveauLoyer,
        loyer_initial: nouveauLoyer,
        irl_reference: irl.valeur,
        date_derniere_revision: todayStr,
      })
      .eq("id", bail.id)
      .eq("proprietaire_id", proprio.id);
