...
function signatureRecapBlock(rows: Array<{ label: string; value: string }>): string {
  const items = rows
    .map(
      (r) =>
        `<tr><td style="padding:6px 0;color:#6b7280;font-size:13px;vertical-align:top;">${escapeHtml(r.label)}</td><td style="padding:6px 0 6px 12px;color:#1a0533;font-size:13px;font-weight:600;">${escapeHtml(r.value)}</td></tr>`,
    )
    .join("");
  return `<div style="margin:20px 0;padding:16px;background:#f8f7ff;border:1px solid #e9d5ff;border-radius:10px;">
...
function signatureEmailShell(title: string, bodyHtml: string, ctaHtml = ""): string {
  return `<div style="background:#f8f7ff;padding:24px;font-family:Arial,Helvetica,sans-serif;">
    <div style="max-width:600px;margin:0 auto;background:#ffffff;border:1px solid #e9d5ff;border-radius:14px;padding:32px;">
      ...
      <h1 style="margin:0 0 20px 0;font-size:22px;font-weight:700;color:#1a0533;text-align:center;">${title}</h1>
      ${bodyHtml}
      ${ctaHtml}
...
export function emailSignatureManualConfirmOwner(params: { signerName: string }): string {
  ...
  const body = `<p style="margin:0 0 14px 0;color:#6b7280;line-height:1.6;font-size:15px;">
      Vous avez confirmé la réception du document signé par <strong style="color:#1a0533;">${signer}</strong>.
...
export function emailSignatureOtpInvite(params: {
  ...
  const contextBlock = params.documentContext?.trim()
    ? `<p style="margin:0 0 20px 0;color:#6b7280;font-size:14px;font-style:italic;line-height:1.6;">Bien concerné : ${escapeHtml(params.documentContext.trim())}</p>`
    : "";

  const body = `<p style="margin:0 0 14px 0;color:#4b5563;line-height:1.6;font-size:15px;">Bonjour <strong style="color:#1a0533;">${name}</strong>,</p>
    <p style="margin:0 0 16px 0;color:#6b7280;line-height:1.6;font-size:15px;">
      <strong style="color:#7c3aed;">${owner}</strong> vous invite à signer votre ${docType} en ligne via Locavio.
    </p>
    <p style="margin:0 0 16px 0;color:#6b7280;line-height:1.6;font-size:15px;">
      Le document complet est joint à cet email en pièce jointe. Prenez le temps de le lire avant de signer.
    </p>
    ${contextBlock}
    <p style="margin:0 0 8px 0;color:#6b7280;line-height:1.6;font-size:14px;">
    ...
    <div style="text-align:center;margin:28px 0;">
      ...
      <p style="color:#6b7280;font-size:12px;margin-top:10px;">Code valable 72 heures</p>
    </div>`;

  const ownerEmail = escapeHtml(params.proprietaireEmail.trim());
  const cta = `<div style="text-align:center;margin:24px 0;">
      <a href="${signLink}" style="background:#7c3aed;color:#ffffff;text-decoration:none;border-radius:10px;padding:14px 28px;font-weight:600;display:inline-block;font-size:15px;box-shadow:0 4px 12px rgba(124,58,237,0.3);">Signer le document →</a>
    </div>
  ...
  const dualChoice = `<div style="margin:32px 0;padding:20px;background:#f8f7ff;border-radius:12px;border:1px solid #e9d5ff;">
    <p style="color:#7c3aed;font-size:13px;font-weight:700;margin:0 0 16px 0;text-transform:uppercase;letter-spacing:1px;">
    ...
    <div style="display:flex;gap:12px;margin-bottom:16px;align-items:flex-start;">
      ...
      <div>
        <p style="color:#1a0533;font-size:13px;font-weight:700;margin:0 0 4px 0;">Option 1 — Signature électronique</p>
    ...
    <div style="display:flex;gap:12px;align-items:flex-start;">
      <div style="width:32px;height:32px;background:#e9d5ff;border-radius:8px;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:16px;">📄</div>
      <div>
        <p style="color:#1a0533;font-size:13px;font-weight:700;margin:0 0 4px 0;">Option 2 — Signature papier</p>
        <p style="color:#6b7280;font-size:12px;margin:0;">
...
export function emailSignatureSignerConfirmation(params: {
  ...
  const body = `<p style="margin:0 0 14px 0;color:#4b5563;line-height:1.6;font-size:15px;">Bonjour <strong style="color:#1a0533;">${name}</strong>,</p>
    <p style="margin:0 0 20px 0;color:#6b7280;line-height:1.6;font-size:15px;">
      Votre signature électronique a bien été enregistrée le <strong style="color:#1a0533;">${escapeHtml(date)}</strong>${time ? ` à <strong style="color:#1a0533;">${escapeHtml(time)}</strong>` : ""}.
...
export function emailSignatureOwnerNotification(params: {
  ...
  const body = `<p style="margin:0 0 14px 0;color:#4b5563;line-height:1.6;font-size:15px;">Bonjour <strong style="color:#1a0533;">${prenom}</strong>,</p>
    <p style="margin:0 0 20px 0;color:#6b7280;line-height:1.6;font-size:15px;">
      <strong style="color:#7c3aed;">${signer}</strong> vient de signer son document électroniquement sur Locavio.
  ...
  const cta = `<div style="text-align:center;margin:24px 0 0;">
      <a href="${docsUrl}" style="background:#7c3aed;color:#ffffff;text-decoration:none;border-radius:10px;padding:14px 28px;font-weight:600;display:inline-block;font-size:15px;box-shadow:0 4px 12px rgba(124,58,237,0.3);">Voir le document →</a>
    </div>`;
